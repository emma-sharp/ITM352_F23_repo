...
var numberOfRows = Math.ceil(products['boards'].length / 3);

for (i = 0; i < numberOfRows; i++) {
    var row = document.createElement("div");
    row.className = "row py-3";

    for (j = i * 3; j < (i + 1) * 3 && j < products['boards'].length + 1; j++) {
...
for (i = 0; i < products['boards'].length; i++) {
    let col = document.getElementById(`display-col-${i}`);
    col.innerHTML = `
        <img src="./images/${products['boards'][i].image}" class="img-thumbnail display-image" alt="${products['boards'][i].name}">
        <p><b>${products['boards'][i].name}</b></p>
        <div class="product-description">
            <p>Price: $${products['boards'][i].price}</p>
            <p>Type: ${products['boards'][i].type}</p>
            <p>In Stock: ${products['boards'][i].quantity_available}</p>
            <p>${products['boards'][i].description}</p>
        </div>
        <label id="product-input-${i}-label">Quantity: </label>
        <input id="product-input-${i}" type="text" value="${ store_params.has('product0')  ? store_params.get('product' + i) : 0}" name="product${i}" onkeyup="checkQuantityTextbox(this, ${products['boards'][i].quantity_available})"></input>
    `
}
    ...
    if (params.has('error')) {
        ...
        for (i = 0; i < products['boards'].length; i++) {
            if (params.has(`product${i}`)) {
                ...
                total_qty += a_qty;
                if(!isNonNegIntAndStock(a_qty, products['boards'][i].quantity_available)) {
                    has_errors = true;
                    checkQuantityTextbox(product_selection_form[`product-input-${i}`], products['boards'][i].quantity_available);
                }
                if (a_qty > products['boards'][i].quantity_available) { stock_error = true; }
            }
        }
